...
void PmergeMe::insertSorted(std::deque<int> & container, int value)
{
    if (container.empty())
    {
...
int PmergeMe::min_value_idx(std::deque<int> & container)
{
    int idx = -1;
    int _max = INT_MAX;
...
void PmergeMe::separateMinMax(std::deque<int> & container,
                              std::deque<int> & mins,
                              std::deque<int> & maxs)
{
    mins.clear();
...
bool PmergeMe::createStack(const std::string & s, std::deque<int> & container)
{
...
void PmergeMe::printStack(std::deque<int> & container) const
{
    if (container.empty())
    {
...
void PmergeMe::insert_sorted_desc_iter(std::deque<int> & container, int value)
{
    std::deque<int>::iterator it = container.begin();
...
void PmergeMe::generate_jacobsthal_sequence(const std::deque<int> & mins,
                                            std::deque<int> & jacob)
{
    jacob.clear();
...
void PmergeMe::generate_jacobsthal_groups(const std::deque<int> & jacob,
...
void PmergeMe::init_positions( std::deque<int> & sorted,
                              std::deque<int> & pos)
{
...
std::size_t PmergeMe::insert_one_min(std::deque<int> & sorted,
                                     int value_to_insert,
                                     std::size_t max_pos_index)
{
    std::deque<int>::iterator begin_it  = sorted.begin();
    std::deque<int>::iterator range_end = begin_it + max_pos_index + 1;

    std::deque<int>::iterator ins_it =
...
void PmergeMe::bump_positions(std::deque<int> & pos, std::size_t ins_index)
{
...
void PmergeMe::push_min(std::deque<int> & sorted,
                        const std::deque<int> & pending,
                        const std::deque<int> & grJacobsthal)
{
    std::deque<int> max_orig = sorted;

    std::deque<int> pos;
    init_positions(sorted, pos);

    std::deque<int>::const_iterator g_it  = grJacobsthal.begin();
    std::deque<int>::const_iterator g_end = grJacobsthal.end();
